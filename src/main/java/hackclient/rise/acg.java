...
public class acg implements abx, InstanceAccess {
    ...
    public void f(int var1, int var2, int var3) {
        if (var3 <= 0) {
            for (abj abj : this.azU) {
                if (agj.c(abj.pL().getX(), abj.pL().getY(), abj.pL().getZ(), 50.0, var1, var2)) {
                    Client.a.k().a(abj.pK());
                }
            }

            for (abk abk : this.azV) {
                if (agj.c(abk.pL().getX(), abk.pL().getY(), abk.pL().getZ(), 17.0, var1, var2)) {
                    if (this.azX == abk) {
                        this.azX = null;
                    } else {
                        this.azX = abk;
