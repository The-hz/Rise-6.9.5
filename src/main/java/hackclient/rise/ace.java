...
public class ace implements abx, InstanceAccess {
    private final ArrayList<abg> azP = new ArrayList<>();
    private final agk azQ = new agk();

    public ace() {
        for (ahc ahc : ahc.values()) {
            this.azP.add(new abg(ahc, ahd.a("language_local", ahc), ahd.a("language_english", ahc)));
        }
    }
    ...
    public void b(int var1, int var2, float var3) {
        ...
        for (int i = 0; i < this.azP.size(); i++) {
            this.azP.get(i).G((i + 1) * 46 + this.azQ.tE());
        }

        air.disable();
        GL11.glPopAttrib();
        RenderUtil.a(
            vector2f.getX() + d0, vector2f.getY(), vector2f1.x - d0, 40.0, this.getStandardClickGUI().axV, abw.BACKGROUND.pV(), true, true, false, false
        );
        gb.MAIN
            .a(16, gd.REGULAR)
    ...
    public void f(int var1, int var2, int var3) {
        Iterator iterator = this.azP.iterator();

        while (iterator.hasNext()) {
            ((abg)iterator.next()).j(var1, var2);
        }
    }
}
