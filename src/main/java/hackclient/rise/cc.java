...
public enum cc {
    ...
    ENTITY_LOOK(S14PacketEntity.S16PacketEntityLook.class, (packet, map) -> {
        ...
        cd cd2 = (cd)map.get(n);
        if (cd2 != null) {
            cd cd3 = new cd(cd2.gW, cd2.gX, cd2.gY, f, f2);
            map.put(n, cd3);
            return map;
        }
        return null;
    ...
    ENTITY_HEAD_LOOK(aa.class, (packet, map) -> {
        ...
        cd cd2 = (cd)map.get(n);
        if (cd2 != null) {
            cd cd3 = new cd(cd2.gW, cd2.gX, cd2.gY, f, cd2.ha);
            map.put(n, cd3);
            return map;
        }
        return null;
    ...
    private final BiFunction<Packet<?>, Map<Integer, cd>, Map<Integer, cd>> gU;
    private static final cc[] $VALUES = cc.cf();



    private cc(Class<? extends Packet<?>> clazz, BiFunction<Packet<?>, Map<Integer, cd>, Map<Integer, cd>> biFunction) {
        this.gT = clazz;
        this.gU = biFunction;
    }

    public Map<Integer, cd> a(Packet<?> packet, Map<Integer, cd> map) {
    ...
    private static cc[] cf() {
        return new cc[]{SPAWN_MOB, ENTITY_TELEPORT, ENTITY_REL_MOVE, ENTITY_LOOK_MOVE, ENTITY_LOOK, DESTROY_ENTITY, ENTITY_HEAD_LOOK};
    }
}
