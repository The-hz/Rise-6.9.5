...
public class ThemeScreen implements Screen, InstanceAccess {
    private final ArrayList<ThemeComponent> azT = new ArrayList<>();
    private ArrayList<ThemeComponent> azU = new ArrayList<>();
    private final ArrayList<ThemeKeyColorComponent> azV = new ArrayList<>();
    private final agk azW = new agk();
    private ThemeKeyColorComponent azX = null;

    public ThemeScreen() {
        for (Themes themes : Themes.values()) {
            this.azT.add(new ThemeComponent(themes));
        }

        for (KeyColors adw : KeyColors.values()) {
            this.azV.add(new ThemeKeyColorComponent(adw));
        }
    }
    ...
    public void b(int var1, int var2, float var3) {
        ...
        for (int i = 0; i < this.azV.size(); i++) {
            ThemeKeyColorComponent abk = this.azV.get(i);
    ...
    public void f(int var1, int var2, int var3) {
        if (var3 <= 0) {
            for (ThemeComponent abj : this.azU) {
                if (GUIUtil.c(abj.pL().getX(), abj.pL().getY(), abj.pL().getZ(), 50.0, var1, var2)) {
                    Client.a.k().a(abj.pK());
                }
            }

            for (ThemeKeyColorComponent abk : this.azV) {
    ...
    public void qw() {
        ...
        for (int i = 0; i < this.azU.size(); i++) {
            ThemeComponent abj = this.azU.get(i);
