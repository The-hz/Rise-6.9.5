...
public final class AlertManager implements aha {
    ...
    public void a(Check var1) {
        aMR.execute(
            () -> {
                String sxxx = var1.N().Y().getName();
                String s1 = sxxx + var1.O().R() + var1.O().S();
                String s2 = "§cRise §c» §c%player%§7 has failed §c%check% §7(§c%type%§7)%dev% (Violations: §c%vl%§7)"
                    .replaceAll("%player%", sxxx)
                    .replaceAll("%check%", var1.O().R())
                    .replaceAll("%type%", var1.O().S())
                    .replaceAll("%dev%", "")
                ...
                NetworkPlayerInfo networkplayerinfo1 = this.mc.getNetHandler().getPlayerInfo(var1.N().Y().getUniqueID());
